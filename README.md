...
- Markdown 实时编辑和预览
- 多套简历模板：现代、极简、商务、柔和、赛博朋克、学院风
- 可调样式：主题色、字体、字号、行距、页面宽度、页面边距、打印页边距等
- 支持上传 `.md` 文件
- 支持插入头像或图片
- 支持导出独立 HTML 文件
- 支持浏览器打印为 PDF
- 自动保存编辑内容到 `localStorage`
- 针对简历场景定制 Markdown 渲染，例如首张图片自动作为头像、`A | B` 分隔符样式化
...
简历模板样式集中在 `src/resumeStyles.ts`，应用界面样式集中在 `src/styles.css`。
